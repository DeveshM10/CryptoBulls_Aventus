 ...
 * 
 * This module provides client-side voice processing functionality
 * that works offline, mimicking the server-side voice-processor.ts API.
 */

import { Asset, Liability } from '../types/finance';

/**
 * Process voice transcripts offline and extract relevant financial data
 */
export function processVoiceInputOffline(transcript: string, type: 'asset' | 'liability'): Asset | Liability {
  if (!transcript) {
...
/**
 * Extract asset-related information from a transcript (client-side version)
 */
function extractAssetData(transcript: string): Asset {
  ...
  // Generate a unique ID for offline assets
  const id = `offline-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
...
/**
 * Extract liability-related information from a transcript (client-side version)
 */
function extractLiabilityData(transcript: string): Liability {
  ...
  let status: 'current' | 'warning' | 'late' = 'current';
  ...
  // Generate a unique ID for offline liabilities
  const id = `offline-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
